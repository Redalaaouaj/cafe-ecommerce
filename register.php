...
<body class="bg-stone-100">
            ...
            ?>
                <div role="alert" class="m-4 rounded border-s-4 border-red-500 bg-red-200 p-4">
                    <div class="flex items-center gap-2 text-red-800">
                        ...
                        <strong class="block font-medium"> Ce login est déjà existe </strong>
                    </div>
                </div> <?php ;
                    } else {
                        $sqlState = $pdo->prepare("INSERT INTO utilisateur values(null, ?, ?, ?)");
                        $sqlState->execute([$login, $password, $created_at]);
    ...
    <form method="post" class="m-12 text-center w-full space-y-5">
        <div>
            <label for="login" class="block font-medium text-gray-700"> Email </label>
        ...
        <input type="submit" name="connecter" value="Register" class="p-3 bg-gray-800 hover:bg-gray-600 rounded-lg text-white">
